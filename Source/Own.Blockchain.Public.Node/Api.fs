...
module Api =
    ...
    let getTradeOrderBookAggregatedHandler (baseAssetHash, quoteAssetHash) : HttpHandler = fun next ctx ->
        task {
            let response =
                Composition.getTradeOrderBookAggregatedApi (AssetHash baseAssetHash, AssetHash quoteAssetHash)
                |> Ok
                |> toApiResponse
    ...
    let getTradeOrderHandler (tradeOrderHash : string) : HttpHandler = fun next ctx ->
        task {
            let response =
                Composition.getTradeOrderApi (TradeOrderHash tradeOrderHash)
                |> toApiResponse
