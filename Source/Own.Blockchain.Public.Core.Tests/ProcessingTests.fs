...
module ProcessingTests =
    ...
    let ``Processing.processChanges PlaceTradeOrder - propagates values into order book``
        ...
        if tradeOrderState.TimeInForce = ImmediateOrCancel && not tradeOrderState.IsStopOrder then
    ...
    [<Fact>]
    let ``Processing.processChanges SetAccountEligibility to false - Trade order cancelled`` () =
        // INIT STATE
        let senderWallet = Signing.generateWallet ()
        let validatorWallet = Signing.generateWallet ()
    ...
    [<Fact>]
    let ``Processing.processChanges SetAssetEligibility to true - Trade order cancelled`` () =
        // INIT STATE
        let senderWallet = Signing.generateWallet ()
        let validatorWallet = Signing.generateWallet ()
